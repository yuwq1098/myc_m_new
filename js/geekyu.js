/*
 * 原生js工具函数
 * 作者Geek.Yu 在2017-04-26 09:07:00 创建了它，
	...
	var geekyu = (function () {
        ...
        geekyu.prototype = {
            ...
            filter: function (callback) {
                ...
                return new geekyu(matchedItems);
            },
            html: function (html) {
            ...
            is: function (selector) {
                ...
                if (typeof selector === 'string') {
                    ...
                    if (selector.nodeType || selector instanceof geekyu) {
            ...
            eq: function (index) {
                ...
                if (index < 0) {
                    ...
                    if (returnIndex < 0) return new geekyu([]);
                    else return new geekyu([this[returnIndex]]);
                }
                return new geekyu([this[index]]);
            },
            append: function () {
                var i, j, k, newChild;
                ...
                for (k = 0; k < arguments.length; k++) {
                  ...
                  for (i = 0; i < this.length; i++) {
                      ...
                      else if (newChild instanceof geekyu) {
            ...
            next: function (selector) {
                if (this.length > 0) {
                    if (selector) {
                        if (this[0].nextElementSibling && $(this[0].nextElementSibling).is(selector)) return new geekyu([this[0].nextElementSibling]);
                        else return new geekyu([]);
                    }
                    else {
                        if (this[0].nextElementSibling) return new geekyu([this[0].nextElementSibling]);
                        else return new geekyu([]);
                    }
                }
                else return new geekyu([]);
            ...
            nextAll: function (selector) {
                ...
                return new geekyu(nextEls);
            },
            prev: function (selector) {
                if (this.length > 0) {
                    var el = this[0];
                    if (selector) {
                        if (el.previousElementSibling && $(el.previousElementSibling).is(selector)) return new geekyu([el.previousElementSibling]);
                        else return new geekyu([]);
                    }
                    else {
                        if (el.previousElementSibling) return new geekyu([el.previousElementSibling]);
                        else return new geekyu([]);
                    }
                }
                else return new geekyu([]);
            ...
            find : function (selector) {
                ...
                return new geekyu(foundElements);
            },
            children: function (selector) {
                ...
                return new geekyu($.unique(children));
            },
            remove: function () {
                for (var i = 0; i < this.length; i++) {
        ...
        (function () {
            ...
            function createMethod(name) {
                geekyu.prototype[name] = function (targetSelector, listener, capture) {
                    var i;
        ...
        $.each = function (obj, callback) {
            ...
            // Don't bother continuing without a callback
            if (!callback) return;
            if (Array.isArray(obj) || obj instanceof geekyu) {
                var i;
                // Array
        ...
        //获取到document的位置
		$.fn.getPos = function(attr){		
			var value=0;
			var iPos=0;
			var i=0;		
			var dom = this[0];			
			while(dom){
				iPos=attr=='left'?dom.offsetLeft:iPos=dom.offsetTop;
				value+=iPos;
